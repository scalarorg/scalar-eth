...
impl Case for BlockchainTestCase {
    fn load(path: &Path) -> Result<Self, Error> {
        ...
        debug!("Load blockchain test case: {:?}", path);
        Ok(Self {
            tests: {
    ...
    fn run(&self) -> Result<(), Error> {
        ...
        self.tests
            ...
            .try_for_each(|case| {
                ...
                //     Arc::new(case.network.clone().into()),
                //     SequencialEvmConfig::new(Arc::new(case.network.clone().into())),
                // );
                let _ = ExecutionStage::new_with_executor(excutor_provider).execute(
                    &provider,
