...
use super::{AccountBasic, EvmAccount, Storage};
...
impl<N: Network> Storage for RpcStorage<N> {
    ...
    fn basic(&self, address: &Address) -> Result<Option<AccountBasic>, Self::Error> {
        if let Some(account) = self.cache_accounts.lock().unwrap().get(address) {
            return Ok(Some(AccountBasic { balance: account.balance, nonce: account.nonce }));
        }
        let nonce =
            self.fetch(|| self.provider.get_transaction_count(*address).block_id(self.block_id))?;
        ...
        if !self.precompiles.addresses().any(|precompile_address| precompile_address == address)
            && balance.is_zero()
            && nonce == 0
            && code.is_empty()
        {
        ...
        let code_hash = if code.is_empty() {
            ...
            self.cache_bytecodes.lock().unwrap().insert(code_hash, code.into());
            Some(code_hash)
        };
        self.cache_accounts.lock().unwrap().insert(
            *address,
            EvmAccount { balance, nonce, code_hash, code: None, storage: AHashMap::default() },
        );
    ...
    fn code_hash(&self, address: &Address) -> Result<Option<B256>, Self::Error> {
        self.basic(address)?;
        Ok(self.cache_accounts.lock().unwrap().get(address).and_then(|account| account.code_hash))
    }
    ...
    fn storage(&self, address: &Address, index: &U256) -> Result<U256, Self::Error> {
        ...
        let value =
            self.fetch(|| self.provider.get_storage_at(*address, *index).block_id(self.block_id))?;
    ...
    fn block_hash(&self, number: &u64) -> Result<B256, Self::Error> {
        ...
        let block_hash = self
            .fetch(|| self.provider.get_block_by_number(BlockNumberOrTag::Number(*number), false))
            .map(|block| block.unwrap().header().hash())?;

        self.cache_block_hashes.lock().unwrap().insert(*number, block_hash);
