...
use crate::executor::parallel::{
    ...
    memory::MvMemory,
    types::{BuildIdentityHasher, MemoryLocation, TxIdx},
};
...
#[derive(Debug, Clone, PartialEq)]
pub(crate) enum EthereumBlockSpecError {
...
#[derive(Debug, Clone, PartialEq)]
pub(crate) enum EthereumTransactionParsingError {
...
pub(crate) fn get_ethereum_gas_price(
    ...
    match tx_type {
        TxType::Legacy | TxType::Eip2930 => {
            tx.gas_price.map(U256::from).ok_or(EthereumTransactionParsingError::MissingGasPrice)
        }
...
impl PevmChain for PevmEthereum {
    ...
    fn build_mv_memory(
        ...
        estimated_locations
            .insert(beneficiary_location_hash, (0..block_size).collect::<Vec<TxIdx>>());
