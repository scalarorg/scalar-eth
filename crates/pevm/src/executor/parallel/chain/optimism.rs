...
use crate::executor::parallel::{
    evm::PevmTxExecutionResult,
    memory::MvMemory,
    types::{BuildIdentityHasher, MemoryLocation},
};
...
impl PevmOptimism {
    /// Optimism Mainnet
    pub fn mainnet() -> Self {
        PevmOptimism { id: NamedChain::Optimism.into() }
    }
}
...
pub(crate) fn get_optimism_fields(
    ...
    let tx_envelope = match op_tx_type {
        ...
        OpTxType::Eip7702 => Signed::<TxEip7702>::try_from(inner).map(|v| OpTxEnvelope::Eip7702(v)),
        OpTxType::Deposit => {
...
impl PevmChain for PevmOptimism {
    ...
    fn build_mv_memory(
        ...
        MvMemory::new(
            txs.len(),
            estimated_locations,
            [block_env.coinbase, revm::L1_FEE_RECIPIENT, revm::BASE_FEE_RECIPIENT],
        )
    }
    // Taivv: 2024-10-11
    // TODO: remove _with_reward_beneficiary
    // Use original revm version, method optimism_with_spec requires single parameter spec_id
    ...
    fn calculate_receipt_root(
        ...
        let receipt_envelopes: Vec<OpReceiptEnvelope> =
            Iterator::zip(txs.txns(), tx_results.iter())
                .map(|(tx, PevmTxExecutionResult { receipt, state })| {
                    let receipt = Receipt {
                    ...
                    Ok(match tx_type {
                        ...
                        OpTxType::Deposit => {
                            let account_maybe =
                                state.get(&tx.inner.from).expect("Sender not found");
