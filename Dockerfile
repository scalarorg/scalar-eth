...
# Build application
COPY . .
RUN cargo build --profile $BUILD_PROFILE --features "$FEATURES" --locked --bin scalar

# ARG is not resolved in COPY so we have to hack around it by copying the
# binary to a temporary location
RUN cp /app/target/$BUILD_PROFILE/scalar /app/scalar

# Use Ubuntu as the release image
FROM ubuntu AS runtime
WORKDIR /app

# Copy scalar over from the build stage
COPY --from=builder /app/scalar /usr/local/bin
...
ENTRYPOINT ["/usr/local/bin/scalar"]
